...
// showToast creates and runs the toast window.  Blocks until the window closes.
static void showToast(const wchar_t *title, const wchar_t *message, DWORD timeoutSec) {
    ...
    int x = wa.right  - TOAST_W - TOAST_MARGIN;
    ...
    SetTextColor(hdc, RGB(241, 245, 249)); // text-primary (#f1f5f9)
    RECT titleRc = {ACCENT_W + 14, 14, TOAST_W - 14, 40};
    DrawTextW(hdc, title, -1, &titleRc, DT_LEFT | DT_SINGLELINE | DT_END_ELLIPSIS);
    SelectObject(hdc, oldFont);
    DeleteObject(titleFont);

    // Message (regular, 12px)
    HFONT msgFont = CreateFontW(
        -14, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
        DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
        CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_SWISS, L"Segoe UI");
    oldFont = (HFONT)SelectObject(hdc, msgFont);
    SetTextColor(hdc, RGB(148, 163, 184)); // text-muted (#94a3b8)
    RECT msgRc = {ACCENT_W + 14, 42, TOAST_W - 14, TOAST_H - 10};
    DrawTextW(hdc, message, -1, &msgRc, DT_LEFT | DT_WORDBREAK | DT_END_ELLIPSIS);
    SelectObject(hdc, oldFont);
    ...
    int finalX = x;
    int steps = 8;
    int slideFrom = finalX + TOAST_W;
    int i;
    for (i = 0; i <= steps; i++) {
        int cx = slideFrom + (finalX - slideFrom) * i / steps;
...
// the user clicks it.  Designed to be called in a helper subprocess
// spawned in the target WTS session.
